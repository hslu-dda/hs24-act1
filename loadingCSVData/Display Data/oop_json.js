...
class Dimension {
  constructor(data, economy) {
    ...
    this.subdimensions = data.subdimensions;
  ...
  draw(x, y) {
    ...
    this.hover = mouseX > topLeft.x && mouseX < bottomRight.x && mouseY > topLeft.y && mouseY < bottomRight.y;
    ...
    let score2018 = map(this.scores[2018], 0, 5, innerHeight, 0);
    let score2021 = map(this.scores[2021], 0, 5, innerHeight, 0);
    let score2024 = map(this.scores[2024], 0, 5, innerHeight, 0);
    ...
    if (DESIGN.showSubdimensions && this.hover) {
      Object.entries(this.subdimensions).forEach(([key, value]) => {
        let mappedVal = map(value, 0, 5, innerHeight, 0);
        stroke(255, 0, 0);
